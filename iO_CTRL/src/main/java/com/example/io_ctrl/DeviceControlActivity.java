...
//                        mBluetoothLeService.txxx("6870331052000068040C3A2333444444634B4647374AFD16");

                        String dataStr = "6870331052000068040C3A2333444444634B4647374AFD16";
                        if(dataStr.length() > 40 ){
                            int count = dataStr.length()/40+1;

                            for(int i=0 ; i<dataStr.length()/40+1 ; i++){

                                mBluetoothLeService.txxx(
                                        dataStr.substring(i*40, ((i+1)*40)>dataStr.length()?dataStr.length():((i+1)*40)));
                                try {
                                    Thread.currentThread();
                                    Thread.sleep(10);
                                } catch (InterruptedException e) {
...
//                            BluetoothContext.mBluetoothLeService.send_io_data("F6");
                        }else{
                            mBluetoothLeService.txxx(dataStr);
                        }
                        try {
                            Thread.currentThread();
                            Thread.sleep(100);
...
//        	}
    ...
    private void displayData( String data1 ) {
        ...
        if( data1.length()==10)
        {
            ...
            Log.d(TAG, "接收到的数据\t"+Arrays.toString(by));
            {
